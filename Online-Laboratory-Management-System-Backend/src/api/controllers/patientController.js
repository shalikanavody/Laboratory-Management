...
const getFilteredPatients = async (req, res) => {
    try {
        // Extracting the search filter from the request body
        const { searchFilter } = req.body;

        // Building the query to filter patients
        const query = searchFilter
            ? { name: { $regex: searchFilter, $options: "i" } } // Example: filtering by name (case-insensitive)
            : {}; // If no searchFilter is provided, return all patients

        // Fetching patients based on the query
        const filteredPatients = await patient.find(query);

        // Sending the filtered patient data as the response
